...
# Botanix Blockchain Node

This project aims to provide installation, running, and maintenance capabilities of **Botanix validator node** for potential and existing Botanix Blockchain backers. The consensus structure of this chain is delegated proof of stake "DPos" and is governed by the symbiosis of Botanix's implementation of go-ethereum and our system contracts [https://github.com/Botanix-Protocol/system-smart-contracts]. This repository has multiple release candidates inline so we recommend checking for updates for better functions and stability.


## Acknowledgements
Botanix blockchain node inherits its core components from heco-chain project by stars-lab which itself is based on geth by the Ethereum foundation.
...
**Note regarding use of GPUs -** GPUs are primarily used in POW consensus chains. Being a DPos Botanix chain has not only more TPS and fast block production but also doesn't need a GPU altogether for its purpose.



## How to become a validator
To back the Botanix blockchain you can become a validator. Full flow to become a validator, you must:
...
* Once "unauthorized validators" warning shows up, go to https://staking.btxtestchain.com/ and click "Become a validator". Fill in all the details in the form, in the "Fee address" field enter the validator wallet address that you imported into your metamask. Proceed further
...
To install the Botanix validator node in ubuntu linux
```bash
  sudo -i
...
```bash
  sudo -i
  git clone https://github.com/Botanix-Protocol/core-blockchain.git
  cd core-blockchain
